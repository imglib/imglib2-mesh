...
public class SortTrove
{
	...
	private static int partition( final TDoubleArrayList a, final int[] index, final int left, final int right )
	{
		int i = left - 1;
		int j = right;
		while ( true )
	...
	// partition a[left] to a[right], assumes left < right
	private static int partition( final TLongArrayList a, final int left, final int right, final Comparator< Long > c )
	{
		int i = left - 1;
		int j = right;
		while ( true )
